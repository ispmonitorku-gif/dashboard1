...
st.write("Sistem otomatis yang mengonversi file raw `Event_table` menjadi Dashboard menggunakan template asli.")

uploaded_file = st.file_uploader("Upload File Event_table (Excel)", type=["xlsx", "xls"])
TEMPLATE_PATH = "Template_OTDR.xlsx"  # Pastikan file template Anda ada di folder yang sama di GitHub
...
if uploaded_file is not None:
    try:
        ...
        # 3. MENGISI KE TEMPLATE EXCEL ASLI
        ...
        def update_table(ws, df, start_row, start_col=1):
            ...
            for r_idx, row in enumerate(df.itertuples(index=False), start_row):
                for c_idx, val in enumerate(row, start_col):
                    ...
                    if ref_cell.has_style: # Copy gaya dari baris pertama template ke baris-baris baru
            ...
            # Hapus data ghosting di bawahnya jika tabel baru lebih pendek
        ...
        st.download_button(
            ...
            file_name="Dashboard_OTDR_Identik.xlsx",
